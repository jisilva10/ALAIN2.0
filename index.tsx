...
| 📊 **Indicadores de desempeño vs. Cultura** | ¿Los KPIs promueven comportamientos congruentes con los valores culturales deseados?                   | Comparación entre sistemas de recompensa y comportamientos culturais visibles. |
...
async function handleSaveEdit(newText: string) {
    ...
    // 2. Truncate history arrays (both storage and UI)
    ...
    // 3. Save the truncated history
    saveChatHistory();

    // 4. Re-initialize the chat session with the now-truncated history
    ...
    // 6. Set input value and trigger the send message flow
...
function renderEditForm(message: Message) {
    ...
    // Use timeout to ensure element is in DOM before focusing and resizing
    setTimeout(() => {
        textarea.focus();
        textarea.style.height = 'auto';
        textarea.style.height = `${textarea.scrollHeight}px`;
...
function applyDictationCorrections(text: string): string {
    const corrections: { [key: string]: string } = {
        ...
        'profectos': 'Profektus',
        'profe tus': 'Profektus',
        'prospectus': 'Profektus',
        'prospectos': 'Profektus',
        'prefectuos': 'Profektus',
...
function loadChat(chatId: string) {
    ...
    editingMessageId = null; // Reset editing state when loading a chat
...
async function handleSendMessage() {
    ...
    if (file && !isFileTypeSupported(file)) {
        ...
        const unsupportedFileMessage = `Lo siento, el formato del archivo que has subido ("${escapeHtml(file.name)}") no es compatible. A’LAIN no puede procesar directamente archivos de Microsoft Office (Word, Excel, PowerPoint) en su formato nativo.

**Por favor, convierte tu archivo a uno de los siguientes formatos compatibles:**
*   **Para documentos de texto:** PDF, TXT, RTF
*   **Para hojas de cálculo:** Guarda los datos como PDF o expórtalos a CSV.
*   **Para presentaciones:** Guarda la presentación como PDF.

También puedo analizar directamente los siguientes tipos de archivos:
*   **Imágenes:** JPG, PNG, WEBP
*   **Audio y Video:** MP3, WAV, MP4, MOV
*   **Otros:** JSON, XML, HTML, CSS, y archivos de código fuente.`;
    ...
    sendBtn.disabled = true;
    attachFileBtn.disabled = true;
    renderMessages(); // To hide edit button while loading
...
async function sendPromptToAI(parts: Part[], thinkingMessageIdToRemove?: string) {
    ...
    if (isGreeting) {
        ...
        try {
            ...
            const resultStream = await currentChatSession.sendMessageStream(sendMessageParams);
            ...
            for await (const chunk of resultStream) { 
                ...
                if (chunkText) {
                 ...
                 if (chunkMetadata?.groundingChunks && chunkMetadata.groundingChunks.length > 0) {
                     if (!collectedGroundingMetadata) {
                         collectedGroundingMetadata = { groundingChunks: [] };
                     }
                     // Naive merge, could be improved to avoid duplicates
                     collectedGroundingMetadata.groundingChunks.push(...chunkMetadata.groundingChunks);
                 }
            }
            aiMessage.text = fullResponseText; 
            ...
            if (collectedGroundingMetadata?.groundingChunks) {
                // Deduplicate URIs before assigning to message
                const uniqueChunks = Array.from(new Map(collectedGroundingMetadata.groundingChunks.map(chunk => [chunk.web?.uri, chunk])).values());
                aiMessage.groundingChunks = uniqueChunks
                    .filter(gc => gc.web?.uri) 
                    .map(gc => ({ 
                        web: {
                            uri: gc.web!.uri!, 
    ...
    sendBtn.disabled = false;
    attachFileBtn.disabled = false;
    renderMessages(); // Final render to show/hide edit button
    chatInput.focus();
}
...
function renderMessages() {
    ...
    chatMessages.forEach((msg, index) => {
        // If this message is being edited, render the form and skip the rest
        ...
        // Add Copy button to all user and AI messages with text
        if (msg.text && (msg.sender === 'user' || msg.sender === 'ai') && !msg.id.endsWith('-ai-thinking')) {
            const copyBtn = document.createElement('button');
            copyBtn.className = 'message-action-btn copy-btn';
            copyBtn.innerHTML = '<i class="fas fa-copy"></i>';
            copyBtn.title = 'Copiar texto';
            copyBtn.setAttribute('aria-label', 'Copiar texto del mensaje');
            copyBtn.onclick = (e) => handleCopyClick(e, msg.text);
            actionsContainer.appendChild(copyBtn);
        }
        
        // Add Edit button ONLY to the last user message, when not loading/editing
        if (msg.sender === 'user' && index === lastUserMessageIndex && !isLoading && !editingMessageId) {
            const editBtn = document.createElement('button');
            editBtn.className = 'message-action-btn edit-btn';
            editBtn.innerHTML = '<i class="fas fa-pencil-alt"></i>';
            editBtn.title = 'Editar mensaje';
            editBtn.setAttribute('aria-label', 'Editar mensaje');
            editBtn.onclick = () => handleEditClick(msg.id);
