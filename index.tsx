...
| **Modelos de análisis de conflicto**                  | Identifica fuentes, estilos de manejo y resoluciones organizacionales.      |
...
// Helper for simple markdown-like parsing
function parseSimpleMarkdown(text: string): string {
    let html = escapeHtml(text)
    // Code blocks (```...```) - handle multiline first
    ...
    // Fix multiple <ul> tags created by the simple regex
    .replace(/<\/ul>\s*<ul>/g, '')
     // Ordered lists (1., 2.)
    .replace(/^\s*(\d+)\.\s+(.*)/gm, '<ol start="$1"><li>$2</li></ol>')
    // Fix multiple <ol> tags
    .replace(/<\/ol>\s*<ol start="\d+">/g, '');

    // Replace newlines with <br> for non-list/preformatted text blocks
    return html.split('\n').map(line => {
        if (line.trim().startsWith('<') && (line.trim().endsWith('</pre>') || line.trim().endsWith('</ul>') || line.trim().endsWith('</ol>'))) {
            return line;
        }
        if (line.trim() === '') return '<br>'; // Keep empty lines
        return line.includes('<br>') ? line : line + '<br>';
    }).join('').replace(/(<br>\s*)+$/, ''); // Join and remove trailing <br>s
}
...
function renderMessages() {
    ...
    chatMessages.forEach(message => {
        ...
        messageContent.innerHTML = parseSimpleMarkdown(message.text);
        ...
        copyBtn.onclick = (e) => handleCopyClick(e, message.text);
...
function renderChatHistory(filter: string = '') {
    ...
    const filteredAndSorted = [...chatHistory]
    ...
    if (filteredAndSorted.length === 0 && filter === '') {
...
function handleCopyClick(event: MouseEvent, textToCopy: string) {
...
function loadChat(chatId: string) {
    ...
    chatMessages = [];
    activeFunction = null; // Reset function on chat load, it will be deduced from history/interaction
    ...
    session.messages.forEach(contentItem => {
        ...
        const message: Message = {
            ...
            groundingChunks: contentItem.groundingMetadata?.groundingChunks?.filter(gc => gc.web?.uri)
        };
...
function initializeAppLogic() {
    ...
    function updateSidebarToggleIcon() {
        ...
        if (icon) {
            if (isClosed) {
                ...
                sidebarToggle.setAttribute('aria-label', 'Abrir barra lateral');
            } else {
    ...
    // Attachment and dictation logic
...
function applyDictationCorrections(text: string): string {
    const corrections: { [key: string]: string } = {
        'prospecto': 'Profektus', 'profectus': 'Profektus', 'perfectos': 'Profektus',
        'prof textos': 'Profektus', 'profectos': 'Profektus', 'profe tus': 'Profektus',
        'prospectus': 'Profektus', 'prospectos': 'Profektus', 'prefectuos': 'Profektus',
        'alaín': 'A’LAIN', 'a lain': 'A’LAIN', 'a laín': 'A’LAIN', 'alain': 'A’LAIN',
        'line': 'A’LAIN', 'a line': 'A’LAIN', 'allain': 'A’LAIN', 'Laín':'A’LAIN',
        'work shop': 'Workshop', 'workshow': 'Workshop', 'worshop': 'Workshop', 'worchot': 'Workshop',
        'lego sirius play': 'Lego® Serious Play®', 'lego serios play': 'Lego® Serious Play®',
        'lego serious play': 'Lego® Serious Play®', 'design thinking': 'Design Thinking',
        'cliente core': 'Client Core', 'clientecore': 'Client Core', 'camba': 'CANVA',
        'canba': 'CANVA', 'foda': 'FODA', 'suave': 'SWOT', 'flying Core': 'Client core'
    };
...
function startDictation() {
    ...
    if (SpeechGrammarListCtor) {
        ...
        const customWords = [
            'Profektus', 'Alain', 'A Lain', 'Workshop', 'Client Core', 'Propuesta', 'Proyecto', 
            'Registro', 'Informe', 'Lego Serious Play', 'Design Thinking', 'Scrum', 'CANVA',
            'SWOT', 'FODA', 'Flow', 'Mihaly Csikszentmihalyi', 'Hersey', 'Blanchard', 'Covey',
            'Maslow', 'Goleman', 'Kotter', 'Vroom', 'GROW', 'SCARF', 'RACI', 'Kaizen'
        ];
...
async function sendPromptToAI(parts: Part[], userMessageId?: string) {
    ...
    addMessageToChat('ai', '', { idSuffix: 'ai-thinking', explicitId: thinkingMessageId });
    const thinkingBubble = document.getElementById(thinkingMessageId)?.querySelector('.message-bubble');
    if (thinkingBubble) {
    ...
    try {
        const responseStream = await currentChatSession!.sendMessageStream({
            message: parts
        });

        for await (const chunk of responseStream) {
            if (thinkingBubble && thinkingBubble.classList.contains('thinking')) {
            ...
            if (chunk.text) {
                ...
                if (!aiMessage) {
                    const id = userMessageId ? `ai-response-to-${userMessageId}` : `ai-msg-${Date.now()}`;
                    addMessageToChat('ai', aiResponseText, { explicitId: id });
                    aiMessage = chatMessages[chatMessages.length - 1];
                } else {
        ...
        addMessageToChat('error', `Error: No se pudo obtener una respuesta. ${errorMessage}`);
    } finally {
        const thinkingMessageElement = document.getElementById(thinkingMessageId);
        if (thinkingMessageElement) {
            chatMessages = chatMessages.filter(m => m.id !== thinkingMessageId);
            renderMessages();
        }
        ...
        if (aiMessage) {
            ...
            if (finalGroundingMetadata?.groundingChunks) {
                aiMessage.groundingChunks = finalGroundingMetadata.groundingChunks
                    .filter(gc => gc.web?.uri)
                    .map(gc => ({ web: { uri: gc.web!.uri!, title: gc.web!.title } }))
                    .slice(0, 4);
            }

            finalizeAIMessage(aiMessage, finalGroundingMetadata);
            
            const activatedFunction = checkAIResponseForFunctionActivation(aiMessage.text);
        ...
        dictateBtn.disabled = isDictating;
        chatInput.focus();
        renderMessages();
    }
}
